...
def analyze_cv_text_file(txt_path):
    txt_file = Path(txt_path)

    if not txt_file.exists():
        raise FileNotFoundError(f"TXT file not found: {txt_path}")

    cv_text = txt_file.read_text(encoding="utf-8", errors="ignore")
    ...
    # filter jobs
    jobs_copy = jobs.copy()
    jobs_copy["normalized_role"] = jobs_copy["job_title"].apply(detect_role)
    jobs_copy["normalized_level"] = jobs_copy.apply(detect_level_from_job, axis=1)

    filtered_jobs = jobs_copy[jobs_copy["normalized_role"] == cv_role].copy()
    if filtered_jobs.empty:
        raise ValueError("No matching jobs found for detected role.")
    ...
    return {
        "cv_file": cv_name,
        "source_field": cv_source_field,
        "detected_field": cv_role,
        "estimated_level": cv_level,
        "relevant_jobs": len(candidate_jobs),
        "companies": unique_companies[:20],
